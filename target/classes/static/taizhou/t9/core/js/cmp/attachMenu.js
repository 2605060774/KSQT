var T9JsAttachMenu = Class.create();
T9JsAttachMenu.prototype = {
  ...
  */
  docMenus:{
    ...
    read : { name:'<div style="display:none">阅读<div>',action:readOffice,extData:'2'},
    ...
    if (extNames["office"].contains(extName)) {
      ...
      if (readOnly) {
        if(canDown == false){
         ...
         return ["read","downFile","dump"];
        }
      }else {
        if(canDown == false){
          return ["read","edit","deleteFile"];
        }else{
          return ["read","downFile","dump","edit","deleteFile"];
        }
      }
      ...
      if (readOnly) {
        return ["play","downFile","dump"];
      }else {
        return ["play","downFile","dump","deleteFile","insertImg"];
      }
      ...
      if (readOnly) {
        ...
        return ["play","downFile","dump","deleteFile"];
      }
      ...
      if (readOnly) {
        ...
        return ["readpdf", "downFile", "dump","deleteFile"];
      }
    }else {
...
function readPdfAttach(dom,cntrlId,extData){
  ...
  var url = contextPath + "/t9/core/funcs/office/ntko/act/T9NtkoAct/readPdf.act?" + param;
  window.open(url);
}
/**
 * 
...
function insertImgc(attachName,attachId,moudle){
  var src = contextPath + "/t9/core/funcs/office/ntko/act/T9NtkoAct/upload.act?attachmentName=" + encodeURIComponent(attachName) + "&amp;attachmentId=" +attachId +"&amp;module="+moudle+"&amp;directView=1" ; 
  try{
    InsertImage(src);
  }catch(e){
    alert("");
...
function attachMenuUtil(container,moduleName,deleteAction,attachNames,attachIds,readOnly,prec , code,isCanPrint,canDown,opFlag,seqId){
  ...
  var attachmenu = new T9JsAttachMenu(cfgs);
  attachmenu.show();
}
/**
 * 自定义浮动菜单类型
