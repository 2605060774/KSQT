...
layui.define('layer', function(exports){
    ...
    var submit = function(){
        ...
        layui.each(verifyElem, function(_, item){
            ...
            var tips = '', value = othis.val(), lens = '';
            othis.removeClass(DANGER);
            layui.each(ver, function(_, thisVerE){
                var arrVer = thisVerE.split(":");
                var thisVer = arrVer[0];
                var isFn = typeof verify[thisVer] === 'function';
                if(thisVer == 'maxlength' && value.length > arrVer[1]){
                    lens = '字符串长度不能大于'+ arrVer[1];
                }else if(thisVer == 'minlength' && value.length < arrVer[1]){
                    lens = '字符串长度不能小于'+ arrVer[1];
                }else if(thisVer == 'rangelength'){/*rangelength:[1,3]*/
                    var arrLen = eval(arrVer[1]);
                    if(value.length < arrLen[0] || value.length > arrLen[1]){
                        lens = '字符串长度需介于'+ arrLen[0] +"和"+ arrLen[1] +"之间";
                    }
                }
                if(lens || verify[thisVer] && (isFn ? tips = verify[thisVer](value, item) : !verify[thisVer][0].test(value)) ){
                    layer.msg(lens || tips || verify[thisVer][1], {
                        icon: 5
                        ,shift: 6
                    });
                    //非移动设备自动定位焦点
