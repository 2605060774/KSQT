    ...
    //渲染视图
        ,render = function(){ //后续性能提升的重点
          ...
          layui.each(data, function(i1, item1){
            ...
            var status = item1.fieldstatus;
            var trStr = '';
            if(status == 1) {
              itemData = "新增<br/>操作人："+item1.updateuser+"<br/>操作时间："+item1.updatetime;
            } else if(status == 2) {
              itemData = "删除<br/>操作人："+item1.updateuser+"<br/>操作时间："+item1.updatetime;
            } else {
              itemData = "修改之前："+item1.oldvalue+"<br/>修改之后："+item1.newvalue+"<br/>操作人："+item1.updateuser+"<br/>操作时间："+item1.updatetime;
            }
            if(status > 1){
              trStr = '<tr class="field-modify-change" data-item="'+itemData+'" data-index="'+ i1 +'">'+ tds.join('') + '</tr>';

              if(status == 2){
                trStr = '<tr class="through-line" data-index="'+ i1 +'">'+ tds.join('') + '</tr>';
              }
            }else{
              trStr = '<tr data-index="'+ i1 +'">'+ tds.join('') + '</tr>';
            }
            trs.push(trStr);
          ...
          /*  修改留痕 */
          //修改后的父元素加上红色边框
          $("tr.field-modify-change").parents(".layui-table-box").addClass("field-parent-red");
          //删除元素加上删除线
          $(".through-line").line();
          //鼠标移上去出现修改信息
          $("tr.field-modify-change").off("mouseenter").on("mouseenter",function(){
  ...
  Class.prototype.events = function(){
    ...
    that.layBody.on('mouseenter','td',function(){
      var othis = $(this)
          ...
          ,scrollWidth = Math.round(elemCell.prop('scrollWidth')) ;
      if(elemCell.hasClass("laytable-cell-radio")){
        scrollWidth -= 2;
      }

      if(scrollWidth > Math.round(elemCell.outerWidth())){
        that.tipsIndex = layer.tips([
          ...
          ,height:'auto'
          ,skin: 'layui-table-newtips'
          ,success: function(layero, index){
            elemCell.on("mouseout",function(){
              layer.close(index);
            })
          }
        });
